...
function openTaskbarIconSettings() {
  try {
    ...
    if (process.platform === 'win32') {
      const scriptPath = path.join(__dirname, 'open-taskbar-settings.ps1');
      const devScriptPath = path.join(app.getAppPath(), 'electron', 'open-taskbar-settings.ps1');
      let targetScript = fs.existsSync(scriptPath) ? scriptPath : (fs.existsSync(devScriptPath) ? devScriptPath : null);

      if (!targetScript) {
        targetScript = path.join(os.tmpdir(), 'cyber-open-taskbar-settings.ps1');
        const embeddedPs = `# Auto-generated by CyberLauncher
...
`;
        try { fs.writeFileSync(targetScript, embeddedPs, 'utf8'); } catch {}
      }

      exec(`powershell.exe -Sta -NoProfile -ExecutionPolicy Bypass -File "${targetScript}"`, { windowsHide: true }, (err) => {
        if (err) {
          console.warn('[SETTINGS] UI Automation navigation error:', err);
        } else {
          console.log('[SETTINGS] UI Automation navigation completed.');
        }
      });
    }
  } catch (err) {
